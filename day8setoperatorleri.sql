...
/* -----------------------------------------------------------------------------
  ...
  verilen sehirleri gosteren sorguyu yaziniz
------------------------------------------------------------------------------*/

    SELECT ýsým  AS isim_veya_sehir,maas
    FROM personel
    WHERE maas>4000
    UNION
    SELECT sehir,maas
...
/* -----------------------------------------------------------------------------
  ...
  yaziniz.
------------------------------------------------------------------------------*/ 
    SELECT ýsým  AS isim_veya_sehir
    FROM personel
    WHERE maas>4000
    UNION ALL
...
------------------------------------------------------------------------------*/
    ...
    SELECT isim,maas AS isim_veya_sehir
    FROM personel
...
------------------------------------------------------------------------------*/      
    SELECT sehir AS sehir_tel ,maas AS maas_cocukKayisi 
    FROM personel
    WHERE id = 123456789
    UNION
...
/* -----------------------------------------------------------------------------
  ORNEK6: Toyota ve Ford sirketlerinde ayni maasi alan  personel varsa listeleyin
------------------------------------------------------------------------------*/    
    SELECT isim, maas
    FROM personel
    WHERE sirket = 'Toyota'
...
------------------------------------------------------------------------------*/ 
    --1.yol Maasi 5000 den az olanlar ile sirketi honda olmayanalari sec
    SELECT ýsým,maas FROM personel
    WHERE maas<5000
    MINUS
    SELECT isim,maas FROM personel
    WHERE sirket = 'Honda';
    
    --2.yol Maasi 5000 den az olanlar ile honda dan farkli olan kayitlarini sec
    SELECT ýsým,maas FROM personel
    WHERE maas<5000
    INTERSECT
    SELECT isim,maas FROM personel
...
------------------------------------------------------------------------------*/    
    ...
    FROM personel
    WHERE isim = 'Mehmet Ozturk'  
    MINUS --sehiri istanbu olmayan farkli olan 
    SELECT isim,sehir FROM personel
    WHERE sehir = 'Istanbul'; --sartta buraya esittir koyduk ancak minus kullandigimiz icin
                              --istanbul olmayan demek istiyor
